...
class SignUpForm(UserCreationForm):
    ...
    id_type = forms.ChoiceField(
        ...
        help_text=_('Required. Enter an existing ID type.'),
    )
    ...
    def clean_id_type(self):
        ...
        dummy = DummyUser.objects.filter(Type_Of_ID=id_type)
        if not dummy:
            raise ValidationError(_('Id type must be Passport, Driving License, StateID.'))
        return id_type

    def clean_id_number(self):
        id_number = self.cleaned_data['id_number']
        dummy = DummyUser.objects.filter(ID_Number=id_number)
        if not dummy:
            raise ValidationError(_('Id number does not exist in dummy govt database'))
        return id_number
